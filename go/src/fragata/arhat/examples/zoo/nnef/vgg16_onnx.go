...
func Vgg16Onnx() *models.Model {
    ...
    variable15 := m.Variable("label", "vgg0_conv7_weight", "shape", []int{512, 256, 3, 3})
    variable16 := m.Variable("label", "vgg0_conv7_bias", "shape", []int{512})
    variable17 := m.Variable("label", "vgg0_conv8_weight", "shape", []int{512, 512, 3, 3})
    ...
    variable27 := m.Variable("label", "vgg0_dense0_weight", "shape", []int{4096, 25088})
    variable28 := m.Variable("label", "vgg0_dense0_bias", "shape", []int{4096})
    variable29 := m.Variable("label", "vgg0_dense1_weight", "shape", []int{4096, 4096})
    variable30 := m.Variable("label", "vgg0_dense1_bias", "shape", []int{4096})
    variable31 := m.Variable("label", "vgg0_dense2_weight", "shape", []int{1000, 4096})
    variable32 := m.Variable("label", "vgg0_dense2_bias", "shape", []int{1000})
    reshape1 := m.Reshape(variable32, "shape", []int{1, -1})
    reshape2 := m.Reshape(variable30, "shape", []int{1, -1})
    ...
    reshape3 := m.Reshape(variable28, "shape", []int{1, -1})
    ...
    relu7 := m.Relu(conv7)
    maxPool3 := m.MaxPool(relu7, "kernel", []int{2, 2}, "stride", []int{2, 2})
    conv8 := m.Conv(maxPool3, variable15, variable16, "kernel", []int{3, 3}, "pads", []int{1, 1, 1, 1})
    relu8 := m.Relu(conv8)
    conv9 := m.Conv(relu8, variable17, variable18, "kernel", []int{3, 3}, "pads", []int{1, 1, 1, 1})
    relu9 := m.Relu(conv9)
    conv10 := m.Conv(relu9, variable19, variable20, "kernel", []int{3, 3}, "pads", []int{1, 1, 1, 1})
    relu10 := m.Relu(conv10)
    maxPool4 := m.MaxPool(relu10, "kernel", []int{2, 2}, "stride", []int{2, 2})
    conv11 := m.Conv(maxPool4, variable21, variable22, "kernel", []int{3, 3}, "pads", []int{1, 1, 1, 1})
    relu11 := m.Relu(conv11)
    conv12 := m.Conv(relu11, variable23, variable24, "kernel", []int{3, 3}, "pads", []int{1, 1, 1, 1})
    relu12 := m.Relu(conv12)
    conv13 := m.Conv(relu12, variable25, variable26, "kernel", []int{3, 3}, "pads", []int{1, 1, 1, 1})
    relu13 := m.Relu(conv13)
    maxPool5 := m.MaxPool(relu13, "kernel", []int{2, 2}, "stride", []int{2, 2})
    reshape4 := m.Reshape(maxPool5, "shape", []int{0, -1})
    matmul1 := m.Matmul(reshape4, variable27, "transB", true)
    add1 := m.Add(matmul1, reshape3)
    relu14 := m.Relu(add1)
    reshape5 := m.Reshape(relu14, "shape", []int{0, -1})
    matmul2 := m.Matmul(reshape5, variable29, "transB", true)
    add2 := m.Add(matmul2, reshape2)
    relu15 := m.Relu(add2)
    reshape6 := m.Reshape(relu15, "shape", []int{0, -1})
    matmul3 := m.Matmul(reshape6, variable31, "transB", true)
