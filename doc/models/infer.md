...
as part of user applications. The source neural network descriptions, referred in this
tutorial as models, can be specified in two forms:

* as [Go](https://golang.org/) program code using Arhat API
* as Khronos [NNEF](https://www.khronos.org/nnef) model description

In this tutorial we will focus on using Arhat API. We plan to cover usage of NNEF
in a separate tutorial in the future.

Conversion of a source Go description into an executable model includes two phases.
In the first phase the comventional Go toolchain is used to compile the model
description and link it to the Arhat SDK libraries to produce an executable for
a C++ code generator. At the second stage this generator is invoked to produce
C++ code for a specified target platform and model variation.
The identification for the target platform and model variation must be passed to
the generator at invocation time. The generator will produce the standalone
inference code in a specified directory. This code can be directly integrated
into a user application or used independently from a command line. This tutorial
covers the independent command line usage.
...
* optional, for extracting weights from torchvision model files:
...
generation. A detailed description of supported variants is provided
in Appendix A at the end of this tutorial.
...
from 10 arbitrarily selected husky images; these images can be
...
The following deep learning models for image classification are currently available.
