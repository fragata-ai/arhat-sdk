...
Performs element-wise greater or equal than comparison **>=** with broadcast support
...
```
func(f *Fragment) Ge(args ...interface{}) front.Layer
```
...
func Ge(g api.Graph, t core.Type, aShape []int, bShape []int) {
    a := g.NewTensor(t)
    b := g.NewTensor(t)
    c := g.NewTensor(core.TypeBool)
    g.External(a, aShape)
    g.External(b, bShape)
    g.GeOp(a, b, c, false, -1)
    g.Return(c)
}
```
...
func Ge(m *models.Model, t string, aShape []int, bShape []int) {
