...
Performs element-wise equal to comparison **==** with broadcast support
...
```
func(f *Fragment) Eq(args ...interface{}) front.Layer
```
...
func Eq(g api.Graph, t core.Type, aShape []int, bShape []int) {
    a := g.NewTensor(t)
    b := g.NewTensor(t)
    c := g.NewTensor(core.TypeBool)
    g.External(a, aShape)
    g.External(b, bShape)
    g.EqOp(a, b, c, false, -1)
    g.Return(c)
}
```
...
func Eq(m *models.Model, t string, aShape []int, bShape []int) {
