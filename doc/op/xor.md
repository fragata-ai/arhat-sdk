...
Performs element-wise logical operation **xor** with broadcast support.
...
```
func(f *Fragment) Xor(args ...interface{}) front.Layer
```
...
func Xor(g api.Graph, t core.Type, aShape []int, bShape []int) {
    a := g.NewTensor(t)
    b := g.NewTensor(t)
    c := g.NewTensor(t)
    g.External(a, aShape)
    g.External(b, bShape)
    g.XorOp(a, b, c, false, -1)
    g.Return(c)
}
```
...
func Xor(m *models.Model, t string, aShape []int, bShape []int) {
